...
**🔒 数据防滥用限制（agent层面）**：为防止通过本 agent 批量拉取/盗取市场数据，fetch_data.py 内置日期范围限制：
- **日期范围限制**：只能拉取最近 30 个自然日内的数据，拒绝拉取远期历史数据（防止批量爬取全量历史）
- 真正的调用频率限流由服务端（财新API层面）按 `CXDA_USER_KEY` 负责，agent层面不做机器级频率限制（多用户共享场景下不合理）
